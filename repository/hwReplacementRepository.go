...
func (repo *repository) GetHwReplacement(request *model.GetHwReplacementRequest) ([]entity.HwReplacement, error) {
	...
	error := repo.db.Raw("SELECT hw_replacement.*, hardware.name AS hw_name FROM hw_replacement LEFT OUTER JOIN hardware ON (hw_replacement.hw_id = CAST(hardware.id AS varchar(10))) WHERE ticket_code = @TicketCode ORDER BY created_at ASC", request).Find(&hw_replacement).Error
