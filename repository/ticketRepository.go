...
func (repo *repository) CheckTicketCode(request string) ([]model.GetTicketResponse, error) {
	...
	error := repo.db.Raw("SELECT ticket.*, users.name as user_name, team.name as team_name, mms_category.name as category_name FROM ticket LEFT OUTER JOIN users ON (ticket.assigned_to = CAST(users.id AS varchar(10))) LEFT OUTER JOIN team ON (ticket.assigned_to_team = CAST(team.id AS varchar(10))) LEFT OUTER JOIN mms_category ON (ticket.category = CAST(mms_category.id AS varchar(10))) WHERE ticket_code = @TicketCode", model.CreateTicketRequest{
		TicketCode: request,
	}).Find(&ticket).Error
