...
func (controller *preventiveController) GetPreventiveActivity(context *gin.Context) {
	var request *model.GetPreventiveActivityRequest

	error := context.ShouldBindJSON(&request)
	description := []string{}
	http_status := http.StatusOK
	var status model.StandardResponse
	var preventive []model.GetPreventiveActivityResponse
	...
	if error != nil {
		...
		preventive, error = controller.preventiveService.GetPreventiveActivity(request)
	...
	parse_request, _ := json.Marshal(request)
	parse_status, _ := json.Marshal(status)
	parse_preventive, _ := json.Marshal(preventive)
	var result = fmt.Sprintf("{\"status\": %s, \"result\": %s}", string(parse_status), string(parse_preventive))
	controller.logService.CreateLog(context, string(parse_request), result, time.Now(), http_status)
}
